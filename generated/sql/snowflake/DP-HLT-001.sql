...
SELECT
  encounter_id,  -- masking policy MASK_PHI attached below
  index_encounter_id,  -- masking policy MASK_PHI attached below
  attributed_member_id,  -- masking policy MASK_PHI attached below
  ed_encounter_id,  -- masking policy MASK_PHI attached below
  discharge_date,  -- masking policy MASK_PHI attached below
  condition,
  unit,
  payer,
  region,
...
ALTER TABLE DP_HLT_001.T_DP_HLT_001 MODIFY COLUMN encounter_id SET MASKING POLICY GOVERNANCE.MASK_PHI;
ALTER TABLE DP_HLT_001.T_DP_HLT_001 MODIFY COLUMN index_encounter_id SET MASKING POLICY GOVERNANCE.MASK_PHI;
ALTER TABLE DP_HLT_001.T_DP_HLT_001 MODIFY COLUMN attributed_member_id SET MASKING POLICY GOVERNANCE.MASK_PHI;
ALTER TABLE DP_HLT_001.T_DP_HLT_001 MODIFY COLUMN ed_encounter_id SET MASKING POLICY GOVERNANCE.MASK_PHI;
ALTER TABLE DP_HLT_001.T_DP_HLT_001 MODIFY COLUMN discharge_date SET MASKING POLICY GOVERNANCE.MASK_PHI;
...
ALTER TABLE DP_HLT_001.T_DP_HLT_001 ADD ROW ACCESS POLICY GOVERNANCE.PURPOSE_BOUND ON (ALL);
...
ALTER TABLE DP_HLT_001.T_DP_HLT_001 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_NOT_NULL ON (encounter_id);

-- QR-HLT-001-02: freshness / partition_completion_by (severity critical)
ALTER TABLE DP_HLT_001.T_DP_HLT_001 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_PARTITION_COMPLETION_BY ON (*);

-- QR-HLT-001-03: validity / in_reference_set (severity critical)
ALTER TABLE DP_HLT_001.T_DP_HLT_001 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_IN_REFERENCE_SET ON (condition);

-- QR-HLT-001-04: uniqueness / unique_at_grain (severity critical)
ALTER TABLE DP_HLT_001.T_DP_HLT_001 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_UNIQUE_AT_GRAIN ON (encounter_id, measure);

-- QR-HLT-001-05: consistency / implies_index_eligible (severity critical)
ALTER TABLE DP_HLT_001.T_DP_HLT_001 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_IMPLIES_INDEX_ELIGIBLE ON (readmitted_within_30d);

-- QR-HLT-001-06: accuracy / reconcile_to_adt (severity high)
ALTER TABLE DP_HLT_001.T_DP_HLT_001 ADD DATA METRIC FUNCTION GOVERNANCE.DMF_RECONCILE_TO_ADT ON (inpatient_days);
